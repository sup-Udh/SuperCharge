...
chrome.tabs.query({}, function (tabs) {
    for (var i = 0; i < tabs.length; i++)
    {
        ...
        if(document.getElementById("lol") != null){
          var BookMarks = document.getElementById("lol").innerHTML=`<a href='" + ${tabs[i].url} + "' target='_blank'>" + "<b>" + ${tabs[i].title} + "</b>" + "</a></br><button style=\"width:100%;height:30px;\" data-title=\""+${tabs[i].title}+"\" data-url=\""+${tabs[i].url}+"\">Bookmark above link</button>`
        }
    } 
    {
...
// window.addEventListener("lol", GetUrls());
window.addEventListener("lol" , GetUrls());
